...
* *Use an email inbox as a feed.* Journalists are already used to skimming through huge piles of dirt with the odd potential gem: the email address of their beat or section, which is usually filled with PR junk but is also where people mail their story ideas or information to. It would be great if we could combine this stream with our regular RSS stream and feeds from newswires like Reuters and AP, so we don't have to check three different apps for the same thing. GMail has inbox feeds, but unfortunately last time I checked they were only providing excerpted messages, which is kind of pointless.
